...
contract NotFairToken is ERC20, Ownable {
    ...
    function mint(address to) external onlyOwner {
        ...
        require(SafeMath.add(totalSupply(), MAX_SUPPLY) <= MAX_SUPPLY, "Exceeds maximum supply");
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        if (!_isOwner() && !_isMinter() && recipient != owner() && recipient != minter) {
            // Проверка на переполнение при сложении
            require(SafeMath.add(amount, balanceOf(recipient)) <= MAX_SUPPLY * MAX_BUY_PERCENTAGE / 100, "Exceeds maximum buy percentage");
        }
